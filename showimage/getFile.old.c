...
XtPointer              callback_data; 
{
    ...
    if(w == XtNameToWidget(drawarea->shell, "*GetFileDialog"))
        ...
        if((fp1 = open(file, O_RDWR)) == -1)
            ...
            strcpy(filename, "Could not open file, no file selected");
            }
...
fprintf(stderr, "width of zoom image = %d\n", width);
fprintf(stderr, "height of zoom image = %d\n", height);
        ...
        zoom_sonar = (unsigned char *) XtCalloc(1, width * height);

        for(i = 0; i < height; i++)
            for(j = 0; j < width; j++)
               {
               zoom_sonar[(i * width) + j] =
                sonar_data[(height_offset+(scansize * i))+(width_offset + j)];
               }
