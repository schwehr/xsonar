...
#define EOIS    (1<<13) /* EOI or EOS detected             */
...
/*
#if defined(BYTE) == FALSE  for compatibility with PEPPER BYTE def'n. 
#endif

#if defined(WORD) == FALSE
#endif

#if defined(DWORD) == FALSE
#endif
*/
...
*/
   ...
   /* digitizer parameters */
   /*----------------------*/
   float    auxVal5;          /* Auxilary value from serial input */    /* 40 */
   ...
   /*------------------------*/
   float    telemFishDepth;   /* meters */                              /* 62 */
   float    telemFishHeading; /* degrees */                             /* 66 */
   float    telemFishPitch;   /* degrees */                             /* 70 */
   float    telemFishRoll;    /* degrees */                             /* 74 */
   float    telemFishAlt;     /* meters - from Q-MIPS altitude tracker*//* 78 */
   float    telemSbotAlt;     /* meters - from subbottom sensor */      /* 82 */
   float    telemSpeedLog;    /* from EM speed log */                   /* 86 */
   WORD     soundVelocity;    /* compute speed of sound * 30 (one-way) */
   
   /* sonar parameters */
   /*------------------*/
   float    ch1_floatRawRange;                                          /* 92 */
   ...
   double   shipLatitude;                                             /* 190 */
   double   shipLongitude;                                            /* 198 */
   
   /* #define nav_telemetry.latitude *((double *)
                                   &nav_telemetry.navEasting) */
   float    navEasting;    /* (x) in meters */                        /* 206 */
   float    navNorthing;   /* (y) in meters */                        /* 210 */
   
   float    cableTension;                                             /* 214 */
   
   float    conductivity;     /* water conductivity */                /* 218 */
   float    navFishHeading;   /* degrees */                           /* 222 */
   float    navFishSpeed;     /* knots */                             /* 226 */
   float    navShipGyro;      /* degrees */                           /* 230 */
   
   float    auxVal4;                                                  /* 234 */
   double   navLongitude;     /* longitude of fish */                 /* 238 */

   WORD     fishLayback;                                              /* 246 */
   BYTE     navFixHour;
   BYTE     navFixMinute;
   BYTE     navFixSeconds;
   BYTE     onLineFlag;
